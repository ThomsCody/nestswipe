...
export default function Login() {
  ...
  // Save token from OAuth callback URL param
  useEffect(() => {
    const tokenParam = searchParams.get("token");
    if (tokenParam) {
      login(tokenParam);
      // Don't navigate here — let the next effect handle it after state updates
    }
  }, [searchParams, login]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gray-50">
      <div className="max-w-sm w-full bg-white rounded-xl shadow-lg p-8 text-center">
        <h1 className="text-2xl font-bold text-gray-900 mb-2">Nestswipe</h1>
        <p className="text-gray-500 mb-8">Find your next apartment together</p>
        <button
