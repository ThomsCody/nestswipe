...
async def _process_message(
    ...
    """Run the extraction pipeline for a single already-fetched Gmail message.

    Returns the number of listings created/updated. With force=True, the
    already-processed guard is bypassed — used to reprocess specific emails
    whose earlier attempt failed for a since-fixed transient reason.
    """
    processed = 0
...
async def process_emails_for_user(user: User, db: AsyncSession) -> int:
    ...
    try:
        ...
        for msg_meta in messages:
            ...
            processed += await _process_message(user, db, msg_meta, msg, minio_client)
...
async def reprocess_emails(user: User, db: AsyncSession, email_ids: list[str]) -> int:
    """Force-reprocess specific Gmail messages, bypassing the already-processed guard.

    Used to recover listings from emails whose first attempt failed for a
    transient reason (e.g. anti-bot rate limiting) that has since cleared.
    Does not touch user.last_email_poll — this is out-of-band from normal polling.
    """
    if not user.gmail_refresh_token or not user.openai_api_key:
        return 0
    ...
    for email_id in email_ids:
        try:
            msg = await asyncio.to_thread(_gmail_get_message, service, email_id)
        except Exception:
            logger.exception("Failed to fetch Gmail message %s", email_id)
            continue
        processed += await _process_message(user, db, {"id": email_id}, msg, minio_client, force=True)
